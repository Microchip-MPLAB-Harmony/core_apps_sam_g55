...
void TASK3_Tasks ( void )
{
    ...
    FLEXCOM7_USART_ReadCallbackRegister(uartReadEventHandler, 0);
    FLEXCOM7_USART_ReadThresholdSet(1);
    ...
    while (status == true)
    {
        ...
        if( xSemaphoreTake( dataRxSemaphore, portMAX_DELAY ) == pdTRUE )
        {
            ...
            FLEXCOM7_USART_Write((uint8_t*)"                      Tsk3-P3 <-\r\n", 34);
            xSemaphoreGive(uartMutexLock);

            /* Toggle an LED if character received is 'L' or 'l' */
            while (FLEXCOM7_USART_Read(&readByte, 1) == true)
            {
